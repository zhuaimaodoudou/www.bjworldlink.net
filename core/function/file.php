...
function upload($input_name, $file_ext = null, $max_width = null, $max_height = null, $watermark = false)
{
    ...
    if (is_array($files['tmp_name'])) { // 多文件情况
        ...
        for ($i = 0; $i < $file_count; $i ++) {
            if (! $files['error'][$i]) {
                //计算md5 @cms88
                $filemd5 = md5_file($files["tmp_name"][$i]);
                
                $mediainfo = $model->findFile('filemd5="'.$filemd5.'"');
                //如果存在直接返回URL
                if( !!$mediainfo ){
                    $array_save_file[] = $mediainfo->url;
                }else{
                    ...
                    if (strrpos($upfile, '/') > 0) {
                        ...
                        $mediaData = array(
                            'storage'=>'local',
                            'title'=>get_filename($upfile),
                            'original'=>$files['name'][$i],
                            'type'=>strstr($files["type"][$i], 'image')?'image':'file',
                            'size'=>$files['size'][$i],
                            'url'=>$upfile,
                            'filemd5'=>$filemd5
                        );
            ...
            if( !!$mediainfo ){
                ...
                if (strrpos($upfile, '/') > 0) {
                    ...
                    $mediaData = array(
                        ...
                        'title'=>get_filename($upfile),
                        'original'=>$files['name'],
                        'type'=>strstr($files["type"], 'image')?'image':'file',
                        'size'=>$files['size'],
                        'url'=>$upfile,
                        'filemd5'=>$filemd5
                    );
...
function qiniu_upload($input_name, $file_ext = null, $max_width = null, $max_height = null, $accessKey, $secretKey, $bucket, $url)
{   
    ...
    if (is_array($files['tmp_name'])) { // 多文件情况
        ...
        for ($i = 0; $i < $file_count; $i ++) {
            if (! $files['error'][$i]) {
                $array_save_file[] = handle_qiniu_upload($files['name'][$i], $files['tmp_name'][$i], $array_ext_allow, $max_width, $max_height);
            }
        }
    } else { // 单文件情况
        if (! $files['error']) {
            $array_save_file[] = handle_qiniu_upload($files['name'], $files['tmp_name'], $files['type'], $array_ext_allow, $max_width, $max_height, $token, $url);
        }
    }
    return $array_save_file;
}
...
// 七牛处理并移动上传文件
function handle_qiniu_upload($file, $temp, $type, $array_ext_allow, $max_width, $max_height, $token, $url)
{
    $file = explode('.', $file); // 分离文件名及扩展
    $file_ext = strtolower(end($file)); // 获取扩展
    ...
    //putFile方法参数依次为token，存储的文件名，真是路径，参数，和文件类型，传入相应的参数即可成功完成上传 
    list($ret,$err) = $uploadManager->putFile($token, $name, $filePath, null, $type, false);
    
    $save_file = 'http://' . $url . '/' .$ret['key'];
