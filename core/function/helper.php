...
function sendsms(array $config, $to, $content)
{
    ...
    if (! isset($config['feige_Account']) || ! isset($config['feige_Pwd']) || ! isset($config['feige_SignId'])) {
        alert_back('短信发送参数配置有误');
    }
    
    $data['apikey'] = $config['feige_Account'];
    $data['secret'] = $config['feige_Pwd'];
    $data['sign_id'] = $config['feige_SignId'];
...
{
    $data['Account'] = $config['feige_Account'];
    $data['Pwd'] = $config['feige_Pwd'];
        ...
        if (strpos($matches, $items) !== false) {
            ...
            switch ($items) {
                ...
                case '||':
                    $bool1 = compareSymbol1($arr[0]);
                    $bool2 = compareSymbol1($arr[1]);
                    $bool3 = compareSymbol1($arr[2]);
                    $flag = $bool1 || $bool2 || $bool3 ? 'if' : 'else';
                    break;
            }
            break;
        }
