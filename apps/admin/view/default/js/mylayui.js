layui.use(['element','upload','laydate','form'], function(){
  ...
  var uploadsInst = upload.render({
	...
	,done: function(res){
	   if(res.code==1){
		   ...
		   var reg = /[\u4E00-\u9FA5-_\w]+(?=\.\w{2,4}\b)/g;
		   var picname = res.data[0].match(reg);
		   html += "<dl><dt><img src='"+sitedir+res.data[0]+"' data-url='"+res.data[0]+"'><br/><input type='text' value='"+picname+"'></dt><dd>删除</dd></dl>";
	   }else{
  	...
  	,allDone: function(obj){
	    ...
	    if(files!=''){
	     ...
	     var reg = /[\u4E00-\u9FA5-_\w]+(?=\.\w{2,4}\b)/g;
		   var picname = files.match(reg);
		   var picsname = picname.join(',');
		   $('#'+des+'name').val(picsname); 
  ...
  var appid = $('.excel').data('appid');
  var timestamp = $('.excel').data('timestamp');
  var signature = $('.excel').data('signature');
  //执行附件上传实例
  var uploadExcelInst = upload.render({
	elem: '.excel' //绑定元素
	,url: uploadurl //上传接口
	,field: 'upload' //字段名称
	,multiple: false //多文件上传
	,accept: 'file' //接收文件类型 images（图片）、file（所有文件）、video（视频）、audio（音频）
    ,exts:'xls|xlsx'//size: 1024*200 //最大允许上传的文件大小
	,before: function(obj){ 
		layer.load(); //上传loading
	}
	,done: function(res){
	   var item = this.item;
	   var des=$(item).data('des');
           var scode = $("#copy_scode option:selected").val();
           if(!scode){
               layer.alert('请先选择要导入的栏目！');
           }
	   	layer.closeAll('loading'); //关闭loading
	   	if(res.code==1){
       		var host = window.location.host;
       		console.log(host);
			$.post("/api.php/index/excel_upload",{ 
				url:res.data[0],
				scode:scode,
				signature:signature,
				timestamp:timestamp,
                ...
                console.log(status);
                 if(data.code==1){
                    layer.msg('上传成功,3秒后刷新',function(){
                          window.location.href=window.location.href;
                	}); 
                }else{
                     layer.msg('上传失败！'+data.data);
                 }
