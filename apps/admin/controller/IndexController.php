...
class IndexController extends Controller
{
    ...
    {
        $qiniu_open = $this->config('qiniu_open');
        $qiniu_access_key = $this->config('qiniu_access_key');
        $qiniu_secret_key = $this->config('qiniu_secret_key');
        $qiniu_bucket     = $this->config('qiniu_bucket');
        $qiniu_url        = $this->config('qiniu_url');

        if($qiniu_open == 1 && !empty($qiniu_access_key) && !empty($qiniu_secret_key) && !empty($qiniu_bucket) && !empty($qiniu_url)){
            $upload = qiniu_upload('upload', null, null, null, $qiniu_access_key, $qiniu_secret_key, $qiniu_bucket, $qiniu_url);
        }else{
