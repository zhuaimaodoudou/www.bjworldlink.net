    ...
    public function add()
    {
        if ($_POST) {
            if (! ! $multiplename = post('multiplename')) {
                ...
                $def2 = post('def2');
                $def3 = post('def3');
                $def1_type = post('def1_type');
                $def2_type = post('def2_type');
                $def3_type = post('def3_type');
            ...
            switch ($submit) {
                ...
                case 'name': // 批量修改栏目名称
                    ...
                    if ($listall) {
                        $name = post('name');
                        foreach ($listall as $key => $value) {
                            $this->model->modSortName($value, $name[$key]);
                        }
                        $this->log('批量修改栏目名称成功！');
                        success('修改成功！', - 1);
                    } else {
                ...
                case 'move':
                    ...
                    $pcode = post('pcode');
                    if (! $list) {
                        alert_back('请选择要移动的栏目！');
                    }
                    if ($pcode =='') {
                        alert_back('请选择目标栏目！');
                    }
                    if(in_array($pcode, $list)){
                        alert_back('目标栏目不能包含在要移动的栏目里！');
                    }
                    if ($this->model->moveSort($list, "pcode='" . $pcode . "'")) {
                        $this->log('移动栏目成功！');
                        success('移动栏目成功！', - 1);
                    } else {
                        alert_back('移动栏目失败！');
                ...
                case 'copy':
                    ...
                    $pcode = post('pcode');
                    if (! $list) {
                        alert_back('请选择要复制的栏目！');
                    }
                    if ($pcode =='') {
                        alert_back('请选择目标栏目！');
                    }

                    if ($this->model->copySort($list, $pcode)) {
                        $this->log('复制栏目成功！');
                        success('复制栏目成功！', - 1);
                    } else {
                        alert_back('复制栏目失败！');
                ...
                case 'copyall':
                    ...
                    if(session('acode') == $acode){
                        alert_back('请选择其他语言！');
                    }
                    if ($this->model->copyAll($listall, $acode)) {
                        $this->log('复制栏目成功！');
                        success('复制栏目成功！', - 1);
                    } else {
                        alert_back('复制栏目失败！');
