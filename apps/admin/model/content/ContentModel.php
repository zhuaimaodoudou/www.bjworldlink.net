...
class ContentModel extends Model
{
    ...
    //查找指定分类下的文章
    public function findRelation($mcode, $scode)
    {
        $fields = array(
            'a.id',
            'b.name as sortname',
    ...
    //获取指定栏目的模型编号
    public function findMcode($scode){
    ...
    // 修改列表页文章标题
    public function modTitle($id, $data)
    {
        return parent::table('ay_content')->autoTime()
            ->in('id', $id)
            ->where("acode='" . session('acode') . "'")
    ...
    {
        $list = parent::table('ay_content')->limit(5000)->column('ico,pics,content');
