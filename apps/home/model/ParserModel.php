...
class ParserModel extends Model
{
    ...
    public function getSortRows($scode)
    {
        ...
        foreach ($arr as $value) {
            array_push($where1, "FIND_IN_SET('$value',subscode)");
        }
        $where2 = array(
            "acode='" . get_lg() . "'",
            'status=1',
    ...
    // 获取分类副名称
    public function getSortSubname($scode)
    {
        $result = $this->getSortList();
        return $result[$scode]['subname'];
    }

    // 获取分类缩略图
    public function getSortIco($scode)
    {
        $result = $this->getSortList();
        return $result[$scode]['ico'];
    }
    ...
    // 获取分类标题
    public function getSortTopTitle($scode)
    {
        $result = $this->getSortList();
        return $result[$scode]['title'];
    }

    // 获取分类描述
    public function getSortDesc($scode)
    {
        $result = $this->getSortList();
        return $result[$scode]['description'];
    }

    // 获取分类描述1
    public function getSortDef1($scode)
    {
        $result = $this->getSortList();
        return $result[$scode]['def1'];
    }
    public function getSortDef2($scode)
    {
        $result = $this->getSortList();
        return $result[$scode]['def2'];
    }
    ...
    public function getLists($scode, $num, $order, $filter = array(), $tags = array(), $select = array(), $fuzzy = true, $start = 1, $lfield = null, $lg = null)
    {
       ...
       if ($ext_table) {
        ...
        if ($scode) {
            ...
            foreach ($arr as $value) {
                array_push($scode_arr, "FIND_IN_SET('$value',a.subscode)");
            }
        }
    ...
    // 多个内容详情，不区分语言，兼容跨语言
    public function getMultContent($ids)
    {
        $field = array(
            'a.*',
            'b.name as sortname',
    ...
    public function getContentPics($id, $field)
    {
        ...
        $result = parent::table('ay_content a')->field($field . ',picsname')
            ->join($join)
            ->where("a.id='$id'")
            ->where('a.status=1')
            ->where("a.date<'" . date('Y-m-d H:i:s') . "'")
    ...
    //当前栏目是否有子栏目，有返回1， 没有返回0 
    public function hasSon($scode){ 
        $pcode = $scode; 
        $result = parent::table('ay_content_sort a')->where('a.pcode='.$pcode) 
            ->where('a.status=1')
            ->find();
        if($result){ 
            return 1 ; 
    ...
    //获取当前栏目父级的父级
    public function ppcode($scode){ 
        $pcode = parent::table('ay_content_sort')->where('scode='.$scode) 
            ->value('pcode');
        $result =  parent::table('ay_content_sort')->where('scode='.$pcode) 
            ->value('pcode');
        return $result;
    }
    ...
    // 获取幻灯片组状态 @mk-slidesort
    public function getSlideSortStatus($gid)
    {
        return parent::table('ay_slide_sort')->where("acode='" . get_lg() . "'")
            ->where("id='$gid'")
            ->find();
    }
