...
class ParserController extends Controller
{
    ...
    public function parserAfter($content)
    {
        ...
        $content = $this->parserSlideSortStatusLabel($content); // 幻灯片组状态 @mk-slidesort
    ...
    public function parserSingleLabel($content)
    {
        ...
        $content = str_replace('{pboot:searchcodestatus}', $this->config('search_check_code') === '0' ? 0 : 1, $content); // 是否开启搜索验证码
    ...
    public function parserNavLabel($content)
    {
        ...
        if (preg_match_all($pattern, $content, $matches)) {
            ...
            for ($i = 0; $i < $count; $i ++) {
                ...
                foreach ($params as $key => $value) {
                    switch ($key) {
                        ...
                        case 'num':
                            ...
                            break;
                        case 'slice':
                            if ($params['slice'] && is_string($value)) {
                                $arr = explode(',', $params['slice']);
                ...
                // 读取指定区间的数据
                if(is_array($slice)){
                    $out_data = array_slice($out_data, $slice[0], $slice[1]);
                }else{
                    $out_data = array_slice($out_data, $slice);
                }
            ...
            for ($i = 0; $i < $count; $i ++) {
                ...
                switch ($matches[1][$i]) {
                    ...
                    case 'topsubname':
                        if (! isset($tcode))
                            $tcode = $this->model->getSortTopScode($sort->scode);
                            $content = str_replace($matches[0][$i], $this->adjustLabelData($params, $this->model->getSortSubname($tcode)), $content);
                        break;
                    case 'topico':
                        if (! isset($tcode))
                            $tcode = $this->model->getSortTopScode($sort->scode);
                            if ($this->model->getSortIco($tcode)) {
                                if (! preg_match('/^http/', $this->model->getSortIco($tcode))) {
                                    $content = str_replace($matches[0][$i], $this->adjustLabelData($params, SITE_DIR . $this->model->getSortIco($tcode)), $content);
                                } else {
                                    $content = str_replace($matches[0][$i], $this->adjustLabelData($params, $this->model->getSortIco($tcode)), $content);
                                }
                            } else {
                                $content = str_replace($matches[0][$i], '', $content);
                            }
                    ...
                    case 'toppic':
                        if (! isset($tcode))
                            $tcode = $this->model->getSortTopScode($sort->scode);
                            if ($this->model->getSortPic($tcode)) {
                                if (! preg_match('/^http/', $this->model->getSortPic($tcode))) {
                                    $content = str_replace($matches[0][$i], $this->adjustLabelData($params, SITE_DIR . $this->model->getSortPic($tcode)), $content);
                                } else {
                                    $content = str_replace($matches[0][$i], $this->adjustLabelData($params, $this->model->getSortPic($tcode)), $content);
                                }
                            } else {
                                $content = str_replace($matches[0][$i], '', $content);
                            }
                    ...
                    case 'toptitle':
                        if (! isset($tcode))
                            $tcode = $this->model->getSortTopScode($sort->scode);
                            $content = str_replace($matches[0][$i], $this->adjustLabelData($params, $this->model->getSortTopTitle($tcode)), $content);
                        break;
                    case 'topdescription':
                        if (! isset($tcode))
                            $tcode = $this->model->getSortTopScode($sort->scode);
                            $content = str_replace($matches[0][$i], $this->adjustLabelData($params, $this->model->getSortDesc($tcode)), $content);
                        break;
                    case 'topdef1':
                        if (! isset($tcode))
                            $tcode = $this->model->getSortTopScode($sort->scode);
                            $content = str_replace($matches[0][$i], $this->adjustLabelData($params, $this->model->getSortDef1($tcode)), $content);
                        break;
                    case 'topdef2':
                        if (! isset($tcode))
                            $tcode = $this->model->getSortTopScode($sort->scode);
                            $content = str_replace($matches[0][$i], $this->adjustLabelData($params, $this->model->getSortDef2($tcode)), $content);
                        break;
                    case 'toplink':
                        if (! isset($tcode)) {
                            $tcode = $this->model->getSortTopScode($sort->scode);
                        ...
                        break;
                    case 'ppcode':
                        $ppcode = $this->model->ppcode($sort->scode); 
                        $content = str_replace($matches[0][$i], $ppcode, $content);  
                        break;
                    case 'parentname':
                        if ($sort->pcode == 0) {
        ...
        if (preg_match_all($pattern, $content, $matches)) {
            ...
            for ($i = 0; $i < $count; $i ++) {
                ...
                if (! ! $data = $this->model->getSelect(escape_string($field))) {
                    if(is_array($slice)){
                        $data = array_slice(explode(',', $data), $slice[0], $slice[1]);
                    } else {
                        $data = array_slice(explode(',', $data), $slice);
                    }
                } else {
                    $data = array();
                }
    ...
    public function parserListLabel($content, $cscode = '')
    {
        ...
        if (preg_match_all($pattern, $content, $matches)) {
            ...
            for ($i = 0; $i < $count; $i ++) {
                ...
                //排除属性值
                if($notlike !== ''){
                    $notlike = explode('|', $notlike);
                    if(count($notlike) == 2){
                        if (preg_match('/^ext_[\w\-]+$/', $notlike[0])) { // 扩展字段
                            $where3[] = "$notlike[0] NOT LIKE '%$notlike[1]%'";
                        }else{
                            $where3[] = "a.$notlike[0] NOT LIKE '%$notlike[1]%'";
                        }
                    }
                }
        ...
        if (preg_match_all($pattern, $content, $matches)) {
            ...
            for ($i = 0; $i < $count; $i ++) {
                ...
                if (array_key_exists('id', $params)) {
                    $data = $this->model->getMultContent(escape_string($id));
                    if(is_array($slice)){
                            ...
                            switch ($matches2[1][$j]) {
                                ...
                                case 'link':
                                    if ($value->outlink) {
                                        ...
                                        $one_html = $this->parserContent($matches2[1][$j], $matches2[0][$j], $one_html, $value, 'link', $scode);
                                    }
                                    break;
                                case 'enclosuresize':
                                    if ($value->enclosure && file_exists(ROOT_PATH . $value->enclosure)) {
                                        $one_html = str_replace($matches2[0][$j], $this->adjustLabelData($params, filesize(ROOT_PATH . $value->enclosure)), $one_html);
                                    } else {
                                        $one_html = str_replace($matches2[0][$j], 0, $one_html);
                                    }
                                    break;
        ...
        if (preg_match_all($pattern, $content, $matches)) {
            ...
            for ($i = 0; $i < $count; $i ++) {
                ...
                if (! ! $rs = $this->model->getContentPics(escape_string($id), $field)) {
                    if(is_array($slice)){
                        $pics = array_slice(explode(',', $rs->$field), $slice[0], $slice[1]);
                        $picsname = array_slice(explode(',', $rs->picsname), $slice[0], $slice[1]); 
                    } else {
                        $pics = array_slice(explode(',', $rs->$field), $slice);
                        $picsname = array_slice(explode(',', $rs->picsname), $slice);
                    }
                    ...
                    for ($j = 0; $j < $count2; $j ++) { // 循环替换数据
                        ...
                        switch ($matches2[1][$j]) {
                            ...
                            case 'src':
                                ...
                                break;
                            case 'name':
                                 $one_html = str_replace($matches2[0][$j], $this->adjustLabelData($params, isset($picsname[$vkey]) ? $picsname[$vkey] : ''), $one_html);
                                break;
                            default:
                                $one_html = str_replace($matches2[0][$j], '', $one_html);
                        }
        ...
        if (preg_match_all($pattern, $content, $matches)) {
            ...
            for ($i = 0; $i < $count; $i ++) {
                ...
                if (! ! $checkboxs = $this->model->getContentCheckbox(escape_string($id), escape_string($field))) {
                    if(is_array($slice)){
                        $data = array_slice(explode(',', $checkboxs), $slice[0], $slice[1]);
                    } else {
                        $data = array_slice(explode(',', $checkboxs), $slice);
                    }
                } else {
                    $data = array();
                }
                    ...
                    for ($j = 0; $j < $count2; $j ++) { // 循环替换数据
                        ...
                        switch ($matches2[1][$j]) {
                            ...
                            case 'text':
                                ...
                                break;
                            case 'key':
                                $one_html = str_replace($this->adjustLabelData($params, $matches2[0][$j]), $property_key, $one_html);
                                break;
                            case 'value':
                                $one_html = str_replace($this->adjustLabelData($params, $matches2[0][$j]), $property_value, $one_html);
                                break;
                        }
                    }
                    $key ++;
    ...
    // 解析广告状态标签 @mk-slidesort
    public function parserSlideSortStatusLabel($content)
    {
        $pattern = '/\{pboot:slidesortstatus(\s+[^}]+)?\}/';
        ...
        if (preg_match_all($pattern, $content, $matches)) {
            ...
            for ($i = 0; $i < $count; $i ++) {
                ...
                if ($result = $this->model->getSlideSortStatus($gid)) {
                    $status_val = $result->status;
                } else {
                    $status_val = 0;
                }
                $content = str_replace($matches[0][$i], $status_val, $content);
            }
                ...
                foreach ($data as $value) { // 按查询数据条数循环
                    ...
                    for ($j = 0; $j < $count2; $j ++) { // 循环替换数据
                        ...
                        switch ($matches2[1][$j]) {
                            ...
                            case 'wapsrc':                                     
                                if ($value->wappic) {                                         
                                    if (! preg_match('/^http/', $value->wappic)) {                                             
                                        $one_html = str_replace($matches2[0][$j], $this->adjustLabelData($params, SITE_DIR . $value->wappic), $one_html);                                         
                                    } else {                                             
                                        $one_html = str_replace($matches2[0][$j], $this->adjustLabelData($params, $value->wappic), $one_html);                                         
                                    }                                     
                                } else {                                         
                                    $one_html = str_replace($matches2[0][$j], '', $one_html);                                     
                                }                                     
    ...
    // 解析自定义SQL循环 V3.2.4 build20230304开始废弃(安全性修复 防shell注入)
        ...
        switch ($label) {
            ...
            case 'presubtitle':
                if ($data->type != 2) // 非列表内容页不解析
                    break;
                if (! ! $pre = $this->model->getContentPre($sort->scode, $data->id)) {
                    $content = str_replace($search, $this->adjustLabelData($params, $pre->subtitle), $content);
                } else {
                    if (isset($params['notext'])) {
                        $content = str_replace($search, $params['notext'], $content);
                    } else {
            ...
            case 'predate':
                if ($data->type != 2) // 非列表内容页不解析
                    break;
                if (! ! $pre = $this->model->getContentPre($sort->scode, $data->id)) {
                    $content = str_replace($search, $this->adjustLabelData($params, $pre->date), $content);
                } else {
                    if (isset($params['notext'])) {
                        $content = str_replace($search, $params['notext'], $content);
                    } else {
            ...
            case 'nextsubtitle':
                if ($data->type != 2) // 非列表内容页不解析
                    break;
                if (! ! $next = $this->model->getContentNext($sort->scode, $data->id)) {
                    $content = str_replace($search, $this->adjustLabelData($params, $next->subtitle), $content);
                } else {
                    if (isset($params['notext'])) {
                        $content = str_replace($search, $params['notext'], $content);
                    } else {
            ...
            case 'nextdate':
                if ($data->type != 2) // 非列表内容页不解析
                    break;
                if (! ! $next = $this->model->getContentNext($sort->scode, $data->id)) {
                    $content = str_replace($search, $this->adjustLabelData($params, $next->date), $content);
                } else {
                    if (isset($params['notext'])) {
                        $content = str_replace($search, $params['notext'], $content);
                    } else {
